...
(and (=> (bvsge s (int_to_pbv k 0)) (bvsge s t)) (=> (bvslt s (int_to_pbv k 0)) (bvsge (bvlshr s (int_to_pbv k 1)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec k))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec k))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(assert (>= k 2))
