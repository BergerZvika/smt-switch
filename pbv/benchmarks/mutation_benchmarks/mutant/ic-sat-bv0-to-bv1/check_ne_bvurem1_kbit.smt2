...
(or (distinct s (int_to_pbv k 0)) (distinct t (int_to_pbv k 0)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec k))) (distinct (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec k))) (distinct (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
