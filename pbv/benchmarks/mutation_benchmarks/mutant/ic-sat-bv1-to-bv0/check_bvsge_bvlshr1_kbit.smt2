...
(and (=> (bvslt s (int_to_pbv k 0)) (bvsge (bvlshr s (int_to_pbv k 1)) t)) (=> (bvsge s (int_to_pbv k 0)) (bvsge s t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec k))) (bvsge (bvlshr s x) t)))
  (=> (exists ((x (_ BitVec k))) (bvsge (bvlshr s x) t)) (SC s t))
  )
 )
)
(check-sat)
