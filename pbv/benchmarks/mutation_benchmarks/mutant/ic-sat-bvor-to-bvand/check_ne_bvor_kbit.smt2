...
(or (distinct s (bvnot (int_to_pbv k 0))) (distinct t (bvnot (int_to_pbv k 0))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec k))) (distinct (bvand x s) t)))
  (=> (exists ((x (_ BitVec k))) (distinct (bvor x s) t)) (SC s t))
  )
 )
)
(check-sat)
