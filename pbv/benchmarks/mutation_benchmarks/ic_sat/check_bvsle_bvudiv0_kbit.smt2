...
(or (= (udivtotal (bvmul s t) s) t) (=> (bvsle t (int_to_pbv k 0)) (bvslt (udivtotal min s) t)))
)

(assert (not
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec k))) (bvsle (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec k))) (bvsle (udivtotal x s) t)) (SC s t))
  )
 )
))
(check-sat)
