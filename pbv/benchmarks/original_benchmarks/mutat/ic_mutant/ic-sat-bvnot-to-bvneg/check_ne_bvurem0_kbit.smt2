...
(or (distinct s (int_to_pbv k 1)) (distinct t (int_to_pbv k 0)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec k))) (distinct (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec k))) (distinct (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
