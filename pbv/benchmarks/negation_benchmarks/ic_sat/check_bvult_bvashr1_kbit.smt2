...
(and (not (and (not (bvult s t)) (bvslt s (int_to_pbv k 0)))) (not (= t (int_to_pbv k 0))))
)

(assert (not
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec k))) (bvult (bvashr s x) t)))
  (=> (exists ((x (_ BitVec k))) (bvult (bvashr s x) t)) (SC s t))
  )
 )
))
(check-sat)
