...
(and (=> (bvslt s (int_to_pbv k 0)) (bvsgt (bvlshr s (int_to_pbv k 1)) t)) (=> (bvsge s (int_to_pbv k 0)) (bvsgt s t)))
)

(assert (not
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec k))) (bvsgt (bvlshr s x) t)))
  (=> (exists ((x (_ BitVec k))) (bvsgt (bvlshr s x) t)) (SC s t))
  )
 )
))
(check-sat)
